...
public abstract class WorkspaceRepository {
    ...
    private void publishVertices(
        ...
        for (ClientApiPublishItem data : publishData) {
            try {
                ...
                checkNotNull(vertex);
                if (SandboxStatusUtil.getSandboxStatus(
                        vertex,
                        workspaceId
                ) == SandboxStatus.PUBLIC && !WorkspaceDiffHelper.isPublicDelete(vertex, authorizations)) {
                    String msg;
        ...
        for (ClientApiPublishItem data : publishData) {
            try {
                ...
                if (SandboxStatusUtil.getSandboxStatus(
                        edge,
                        workspaceId
                ) == SandboxStatus.PUBLIC && !WorkspaceDiffHelper.isPublicDelete(edge, authorizations)) {
                    String error_msg;
                    if (data.getAction() == ClientApiPublishItem.Action.DELETE) {
    ...
    private void publishVertex(
        ...
        if (action == ClientApiPublishItem.Action.DELETE || WorkspaceDiffHelper.isPublicDelete(
                vertex,
                authorizations
        )) {
